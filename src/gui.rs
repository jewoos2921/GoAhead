    ...
    match ctx.key {
        ...
        Some(key) => {
            match key {
                VirtualKeyCode::Escape => { (ItemMenuResult::Cancel, None) }
                _ => {
                    let section = rltk::letter_to_option(key);
                    if section > -1 && section < count as i32 {
                        return (ItemMenuResult::Selected, Some(equippable[section as usize]));
                    }
                    (ItemMenuResult::NoResponse, None)
                }
            }
