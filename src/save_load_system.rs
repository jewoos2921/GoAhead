...
use super::{Position, SerializeMe, World, Viewshed, Monster, Map, Name, Ranged, InflictsDamage,
            AreaOfEffect, Confusion, ProvidesHealing, InBackPack, WantsToPickupItem, WantsToUseItem,
            WantsToDropItem, Renderable, Player, BlocksTile, CombatStats, SufferDamage, WantsToMelee,
            Item, Consumable, SerializationHelper, MAP_COUNT};


macro_rules! serialize_individually {
    ($ecs: expr, $ser: expr, $data: expr, $($type: ty), *) => {
        $(SerializeComponent::<NoError, SimpleMarker<SerializeMe>>::serialize(
...
macro_rules! deserialize_individually {
    ($ecs: expr, $de: expr, $data: expr, $($type: ty), *) => {
        $(DeserializeComponent::<NoError,_>::deserialize(
