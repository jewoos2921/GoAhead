...
impl<'a> System<'a> for MeleeCombatSystem {
    type SystemData = (Entities<'a>, WriteExpect<'a, GameLog>, WriteStorage<'a, WantsToMelee>,
                       ReadStorage<'a, Name>, ReadStorage<'a, CombatStats>,
    ...
    fn run(&mut self, data: Self::SystemData) {
        let (entities, mut log,
            mut wants_melee,
            names, combat_stats, mut inflict_damage) = data;
        ...
        for (_entity, wants_melee, name, stats) in (&entities, &wants_melee,
            ...
            if stats.hp > 0 {
                ...
                if target_stats.hp > 0 {
                    ...
                    if damage == 0 {
                        log.entries.push(format!("{} is unable to hurt {}", &name.name,
                                                 &target_name.name));
                    } else {
