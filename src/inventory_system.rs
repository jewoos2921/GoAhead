...
impl<'a> System<'a> for ItemUseSystem {
    ...
    fn run(&mut self, data: Self::SystemData) {
        ...
        for (entity, useitem) in (&entities, &wants_use).join() {
            ...
            // If its a consumable, We delete it one use.
            if used_item {
                let consumable = consumable.get(useitem.item);
                match consumable {
                    None => {}
                    Some(_) => {
            ...
            let item_equippalbe = equippable.get(useitem.item);
            match item_equippalbe {
                None => {}
                Some(can_equip) => {
                    ...
                    for (item_entity, already_equiped, name) in (&entities, &equipped, &names).join() {
                        if already_equiped.owner == target && already_equiped.slot == target_slot {
                    ...
                    if target == *player_entity {
                        gamelog.entries.push(format!("You equip {}.", names.get(useitem.item)
                            .unwrap().name));
                    }
                }
            }
